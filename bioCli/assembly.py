...
def orf_finder(sequence: str) -> list[str]:
    """Trova ORF proteiche gia' rappresentate come M...*.

    Mantiene compatibilita' con il vecchio nome del tool: se l'input e'
    gia' una sequenza aminoacidica, restituisce i tratti dalla M al primo *.
    """
    return protein_stretch_finder(sequence)


def genome_assembly(kmers: list[str]) -> str:
    """Assemblaggio semplice tramite sovrapposizione greedy."""
    return greedy_kmer_assembly(kmers)
