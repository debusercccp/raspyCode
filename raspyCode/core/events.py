...
@dataclass
class FallbackModeEvent(Event):
    """Attivazione/disattivazione del fallback su Ollama locale (127.0.0.1).

    Pubblicato da ConnectivityService dopo un debounce (N healthcheck falliti
    consecutivi per attivarlo, M riusciti consecutivi per disattivarlo), cosi'
    da non reagire a semplice flicker di rete sul link col Pi.
    """
    active: bool
    detail: str = ""
...
class UserChatEvent(Event):
    """Compatibilita' per il flusso RAG storico: query utente senza mittente."""
    query: str


@dataclass
class EnrichedChatEvent(Event):
    """Prompt arricchito dal servizio RAG prima dell'invio al gateway."""
    prompt: str
