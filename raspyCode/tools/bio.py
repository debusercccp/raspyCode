...
def _extract_args(arguments: dict[str, Any]) -> list[Any]:
    ...
    return [v for v in arguments.values() if v is not None]
...
def _simple_tool(
    ...
    async def handler(arguments: dict[str, Any]) -> tuple[str, bool]:
        try:
            ...
            return f"Errore esecuzione tool '{name}': {exc}", True
...
def build_bio_tools() -> list[ToolDefinition]:
    return [
        ...
        _simple_tool(
            ...
            "Trova ORF/tratti M...* in una sequenza proteica.",
            lambda args: f"ORF trovati: {bioCli.orf_finder(args[0] if args else '')}",
        ),
        _simple_tool(
            "biotoolkit_genome_assembly",
            "Assembla una lista di k-mer tramite sovrapposizione greedy.",
            lambda args: f"Genoma assemblato: {bioCli.genome_assembly(args)}",
        ),
        _simple_tool(
        ...
        _simple_tool(
            ...
            "Alias compatibile per il campionamento FASTA.",
            lambda args: bioCli.fastx_sampler(
                args[0] if args else "", float(args[1]) if len(args) > 1 else 100.0,
