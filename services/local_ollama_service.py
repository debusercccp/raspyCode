...
class LocalOllamaService:
    ...
    def _find_ollama_executable() -> str | None:
        """Trova Ollama anche quando non e' nel PATH ereditato dalla TUI.

        Questo capita spesso quando Ollama viene installato come servizio
        desktop (macOS/Homebrew) o tramite installer Linux: il terminale da
        cui parte raspyCode puo' avere un PATH diverso da quello della shell
        interattiva.
        """
    ...
    async def ensure_running(self) -> bool:
        ...
        async with self._start_lock:
            ...
            if self._process is None or self._process.returncode is not None:
                ...
                await self._bus.publish(
                    StatusEvent(
                        text=f"Ollama locale avviato automaticamente ({executable}).",
                        level="info",
                    )
                )
