...
async def main(*, resume: bool = False) -> None:
    ...
    session = load_session(Path.cwd()) if resume else None
    pi_ip = os.environ.get("RASPY_PI_IP", session.get("pi_ip", "10.42.0.2") if session else "10.42.0.2")
    model = os.environ.get("RASPY_MODEL", session.get("model") if session else None)
    resumed_history = session.get("history", []) if session else None
    ...
    app = RaspyCodeApp(bus, pi_ip=pi_ip, model=model, initial_history=resumed_history)
    gateway = LLMGatewayService(bus, pi_ip=pi_ip, model=model, tool_schemas=tool_schemas, initial_history=resumed_history)
...
def start() -> None:
    ...
    parser = argparse.ArgumentParser(prog="raspycode")
    parser.add_argument("--resume", action="store_true", help="riprende la sessione salvata nella directory corrente")
